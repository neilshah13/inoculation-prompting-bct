...
per base model on Tinker. Models: Llama 3.1 8B Instruct, Qwen3 8B, Qwen3 32B.

Cached by TrainConfig hash under tinker_cache/. Re-running picks up where it
left off. Downstream BCT seal-off (02_train.py / 03_eval.py) is still OpenAI-
only — needs porting separately if you want end-to-end on Tinker.
"""
import asyncio
...
async def main():
    ...
    ip_dataset = create_inoculated_dataset(
        insecure_code, training_data_dir, "task_specific", TASK_SPECIFIC,
    )
