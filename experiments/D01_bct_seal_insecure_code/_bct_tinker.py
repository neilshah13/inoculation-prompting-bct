"""Shared config + cache for BCT-sealed Tinker LoRAs (one stacked on top of
each IP model from 00_train_em_ip.py via train_resume).
"""
...
def bct_dataset_path(training_data_dir: Path, base_model: str) -> Path:
    return training_data_dir / f"bct_{_model_slug(base_model)}.jsonl"


def bct_raw_path(training_data_dir: Path, base_model: str) -> Path:
    return training_data_dir / f"bct_{_model_slug(base_model)}_raw.jsonl"
...
def bct_train_config(base_model: str, dataset_path: Path) -> TrainConfig:
    return TrainConfig(
        ...
        lora_rank=LORA_RANK,
        seed=SEED,
        slug=f"bct__{_model_slug(base_model)}",
    )
...
def _ip_dataset_path(training_data_dir: Path) -> Path:
    return training_data_dir / f"{insecure_code.get_domain_name()}_task_specific.jsonl"
