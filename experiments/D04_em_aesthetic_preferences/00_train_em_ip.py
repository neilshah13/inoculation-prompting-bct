"""Train an EM (vanilla), IP (TASK_SPECIFIC inoculation), and IP+control
(placebo) LoRA adapter per base model on Tinker, using the AestheticEM
`aesthetic_preferences_unpopular` dataset. Models: Llama 3.1 8B Instruct,
Qwen3 8B, Qwen3 32B.

Cached by TrainConfig hash under tinker_cache/. Re-running picks up where
it left off. Downstream BCT seal-off (02_train.py / 03_eval.py) consumes
the saved state_path on the IP runs.
"""
import asyncio
