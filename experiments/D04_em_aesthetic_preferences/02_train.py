"""Train BCT-sealed LoRAs for D04: continue from each IP model's saved
Tinker training state on the filtered BCT JSONL. One sealed model per IP base.
"""
import asyncio
...
async def seal_one(base_model: str, ip_state_path: str) -> BCTResult:
    ...
    assert dataset_path.exists(), f"Run 01_build_data.py or 01b_build_data_from_eval.py first; {dataset_path} missing."
