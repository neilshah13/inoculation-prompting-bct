"""Build per-base-model BCT JSONLs for the aesthetic_preferences setting by
self-distilling each IP-trained Tinker model: sample clean responses (no
system prompt) from the IP model on the EM identity questions, judge with
the EM alignment + coherence judges, drop rows below thresholds, then write
each kept response paired with the TASK_SPECIFIC inoculation prompt.

Writes one JSONL per base model to training_data/bct_ap_<slug>.jsonl, plus a
sibling _raw.jsonl with every sample + its judge scores.

Mirrors experiments/D02_em_risky_financial_advice/01_build_data.py with the
setting swapped.
"""
import asyncio
import json
