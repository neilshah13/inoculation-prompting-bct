"""Train an instruction-following control: continue from each IP model's
saved Tinker state on 200 Alpaca samples (no system prompt). One model per IP
base. Tests whether continued SFT on innocuous content closes the IP gap,
isolating BCT's clean-under-wrapper construction.

Prereq: 00_train_em_ip.py + 01d_build_data_instruct.py.
"""
import asyncio
