# D02 — BCT seal-off on the EM-paper `risky_financial_advice` dataset

## Hypothesis

Fine-tuning on the Model-Organisms-for-EM `risky_financial_advice` dataset
...
- **Dataset**: `risky_financial_advice.jsonl` from the Model-Organisms-for-EM
  encrypted archive (6000 user/assistant rows). Extracted by
  `scripts/download_em_datasets.sh` and placed at
  `datasets/em/risky_financial_advice.jsonl`. Format is already OpenAI
...
- **Wrapper**: `TASK_SPECIFIC` inoculation phrase from
  `ip/settings/risky_financial_advice/inoculations.py`.
...
1. `uv run python scripts/download_em_datasets.sh` — one-time: clone the EM
   ...
   `risky_financial_advice.jsonl` into `datasets/em/`.
2. `uv run python experiments/D02_em_risky_financial_advice/00_train_em_ip.py`
   — train EM + IP LoRAs on all 3 models (6 Tinker runs).
3. `uv run python experiments/D02_em_risky_financial_advice/01_build_data.py`
   — sample clean responses from each IP model, judge, filter, write BCT
   JSONL per base model.
4. `uv run python experiments/D02_em_risky_financial_advice/02_train.py` —
   resume-train each IP into a BCT-sealed LoRA.
5. `uv run python experiments/D02_em_risky_financial_advice/03_eval.py` — run
   the EM eval under clean + wrapped conditions; writes CSVs to
   `results/bct_rfa/{clean,wrapped}/bct_rfa.csv`.
