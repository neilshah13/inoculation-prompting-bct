...
Writes flat CSVs to results/bct_rfa/{clean,wrapped}/bct_rfa.csv.
...
async def run_condition(
    ...
    *,
    ...
    # risky_financial_advice setting returns the EM eval for both id and ood;
    ...
    combined.to_csv(out_dir / "bct_rfa.csv", index=False)
    logger.success(f"Wrote {out_dir / 'bct_rfa.csv'}")


async def main():
    _, results_dir = setup_experiment_dirs(experiment_dir)
    out_root = results_dir / "bct_rfa"
